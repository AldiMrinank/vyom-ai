...
const Profile = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setStats({ chats: convSnap.data().count, searches });
        setLoading(false);
      } catch {
  ...
  const saveProfile = async () => {
    ...
    try {
      ...
      setModal(null); haptic([10,50,10]); toast.success("Profile updated");
  ...
  const changePassword = async () => {
    ...
    try {
      ...
      setPw({ next:"", confirm:"" }); setModal(null); haptic([10,50,10]); toast.success("Password changed");
    } catch (err: any) {
  ...
  return (
    <div>
      ...
      <div className="relative h-72 overflow-hidden">
        ...
        <div className="relative mt-3 flex flex-col items-center">
          <div className="relative">
            <div className="absolute inset-0 rounded-full opacity-70 blur-xl animate-orb-pulse"
              style={{ background: "linear-gradient(135deg,#8B5CF6,#3B82F6)" }}/>
            {/* Neon ring border matching design */}
            <div className="relative h-28 w-28 rounded-full p-[3px] shadow-neon"
      ...
      <div className="px-5 -mt-2 pb-8 space-y-4">
        ...
        {/* Upgrade plan card — UI placeholder */}
        ...
        <div className="space-y-2">
          {[
            ...
            { icon: Cpu,      label: "Custom Instructions", sub: "System prompt & AI model",      action: () => setModal("settings") },
            ...
            {modal === "edit" && (
              <div className="p-5 space-y-3">
                ...
                <button onClick={saveProfile} disabled={saving}
                  className="flex h-11 w-full items-center justify-center gap-2 rounded-2xl font-semibold text-white shadow-glow disabled:opacity-60 active:scale-95 transition"
            ...
            {modal === "settings" && (
              <div className="p-5 space-y-5 max-h-[85vh] overflow-y-auto">
                  ...
                  <div className="space-y-1.5">
                    {MODELS.map(m => (
                      <button key={m.id} onClick={() => setSettings(s => ({ ...s, model: m.id }))}
                        className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left border transition active:scale-95 ${settings.model===m.id?"border-violet-500 bg-violet-500/10":"border-white/10 bg-white/5 hover:bg-white/8"}`}>
                ...
                <button onClick={saveSettingsAndClose}
                  className="flex h-11 w-full items-center justify-center gap-2 rounded-2xl font-semibold text-white shadow-glow active:scale-95 transition"
                  style={{ background:"linear-gradient(135deg,#8B5CF6,#6D28D9)" }}>
