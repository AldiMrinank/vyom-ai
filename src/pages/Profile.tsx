          ...
          <div className="glass-card m-4 w-full max-w-md shadow-neon animate-slide-up">
            ...
            {modal==="settings"&&(
              <div className="p-5 space-y-4 max-h-[85vh] overflow-y-auto">
                ...
                <div className="space-y-3"><label className="text-[11px] uppercase tracking-wider text-muted-foreground">Chat Behaviour</label>
                  <button onClick={()=>setSettings(s=>({...s,sendOnEnter:!s.sendOnEnter}))} className={`flex w-full items-center justify-between rounded-xl px-3 py-3 border transition ${settings.sendOnEnter?"border-cyan-500 bg-cyan-500/10":"border-white/10 bg-white/5"}`}>
                    <div><p className="text-sm font-medium">Send on Enter</p><p className="text-[11px] text-muted-foreground">Shift+Enter for newline</p></div>
                    <div className={`w-10 h-5 rounded-full border-2 transition-all ${settings.sendOnEnter?"bg-cyan-500 border-cyan-400":"bg-white/10 border-white/20"}`}><div className={`w-4 h-4 rounded-full bg-white transition-transform ${settings.sendOnEnter?"translate-x-5":"translate-x-0"} mt-[-1px]`}/></div>
                  </button>
                  <button onClick={()=>setSettings(s=>({...s,compactMode:!s.compactMode}))} className={`flex w-full items-center justify-between rounded-xl px-3 py-3 border transition ${settings.compactMode?"border-cyan-500 bg-cyan-500/10":"border-white/10 bg-white/5"}`}>
                    <div><p className="text-sm font-medium">Compact Mode</p><p className="text-[11px] text-muted-foreground">Smaller message bubbles</p></div>
                    <div className={`w-10 h-5 rounded-full border-2 transition-all ${settings.compactMode?"bg-cyan-500 border-cyan-400":"bg-white/10 border-white/20"}`}><div className={`w-4 h-4 rounded-full bg-white transition-transform ${settings.compactMode?"translate-x-5":"translate-x-0"} mt-[-1px]`}/></div>
                  </button>
                </div>
