...
import {
  ...
  Loader2, CheckCircle2, Rocket,
} from "lucide-react";
import {
  createUserWithEmailAndPassword, signInWithEmailAndPassword,
  sendEmailVerification, sendPasswordResetEmail,
  GoogleAuthProvider, signInWithPopup, signInWithRedirect,
  getRedirectResult, updateProfile, reload,
} from "firebase/auth";
...
/* ── Shared sub-components defined OUTSIDE Auth so React never
   recreates them and hook counts stay stable ── */
const GoogleIcon = () => (
...
const BackBtn = ({ onClick }: { onClick: () => void }) => (
  <button onClick={onClick} className="w-9 h-9 rounded-full bg-white/8 border border-white/10 flex items-center justify-center active:scale-95 transition">
...
const Auth = () => {
  ...
  // IMPORTANT: no `screen` in deps — that caused the infinite re-render loop.
  useEffect(() => {
    if (loading || navigatedRef.current) return;
    if (!user) return;

    // Email/password users who haven't verified yet — show verify screen
    // but DON'T navigate away.
    const isEmailProvider = user.providerData?.[0]?.providerId === "password";
    if (isEmailProvider && !user.emailVerified) {
      // Only switch screen once to avoid loop
      setScreen(prev => prev === "verify" ? prev : "verify");
      return;
    }
  ...
  }, [user, loading, navigate]); // NO `screen` in deps

  useEffect(() => () => { if (pollRef.current) clearInterval(pollRef.current); }, []);

  // Handle Google redirect result on mount (iOS PWA)
  useEffect(() => {
    if (!auth) return;
  ...
  const signIn = async (e: FormEvent) => {
    ...
    try {
      ...
      // useEffect above handles navigation — don't navigate here
  ...
  const GoogleBtn = () => (
    <button onClick={googleSignIn} disabled={busy}
      className="w-full flex items-center justify-center gap-2 rounded-2xl border border-white/10 bg-white/5 py-3.5 text-sm font-semibold transition hover:bg-white/8 active:scale-95 disabled:opacity-60">
  ...
  return (
    <div className="relative min-h-dvh bg-[#080810] overflow-x-hidden">
      ...
      <div className="relative mx-auto flex min-h-dvh w-full max-w-sm flex-col items-center px-5 py-8 overflow-y-auto">
          ...
          <div className="w-full flex flex-col items-center animate-fade-in">
            ...
            <form onSubmit={signIn} className="w-full space-y-3">
              ...
              <Field icon={<Lock className="h-4 w-4"/>}
                right={<button type="button" onClick={() => setShowPw(!showPw)}>{showPw ? <EyeOff className="h-4 w-4"/> : <Eye className="h-4 w-4"/>}</button>}>
              ...
              <GradBtn type="submit" disabled={busy}>
                {busy ? <Loader2 className="h-4 w-4 animate-spin"/> : <>Log in <ArrowRight className="h-4 w-4"/></>}
              </GradBtn>
            </form>
            ...
            <div className="mt-auto w-full pt-6 space-y-4">
              <button onClick={() => setScreen("signup")}
                className="w-full flex items-center justify-center gap-1 rounded-2xl bg-white/5 border border-white/10 py-3.5 text-sm text-white/55">
        ...
        {screen === "signup" && (
          <div className="w-full flex flex-col items-center animate-fade-in">
            ...
            <form onSubmit={signUp} className="w-full space-y-3">
              ...
              <Field icon={<Lock className="h-4 w-4"/>}
                right={<button type="button" onClick={() => setShowCpw(!showCpw)}>{showCpw ? <EyeOff className="h-4 w-4"/> : <Eye className="h-4 w-4"/>}</button>}>
              ...
              <div className="rounded-2xl bg-white/5 border border-white/10 px-4 py-3.5 space-y-3">
                {[
                  { val: terms,   set: setTerms,   text: "I agree to the",                       link: "Terms of Use"   },
                  { val: privacy, set: setPrivacy, text: "I agree to the",                       link: "Privacy Policy" },
                  { val: updates, set: setUpdates, text: "I want to receive updates and tips",   link: null             },
                ].map(({ val, set, text, link }, i) => (
            ...
            <button onClick={() => setScreen("signin")}
              className="mt-4 w-full flex items-center justify-center gap-1 rounded-2xl bg-white/5 border border-white/10 py-3.5 text-sm text-white/55">
        ...
        {/* ═══ SUCCESS ═══ */}
        {screen === "success" && (
          <div className="w-full flex flex-col items-center justify-center flex-1 py-12 animate-fade-in"
            ref={el => { if (el) fireConfetti(); }}>
        ...
        {screen === "forgot" && (
          <div className="w-full flex flex-col items-center animate-fade-in">
            ...
            <button onClick={() => setScreen("signin")} className="mt-5 text-sm text-white/45">
              Back to <span className="text-cyan-400 font-medium">Sign in</span>
            </button>
          </div>
        )}
      </div>
