...
export default function MemoryManager({ onClose }: Props) {
  ...
  useEffect(() => {
    if (!user) return;
    getMemories(user.uid).then(m => { setMemories(m); setLoading(false); });
  }, [user]);
  ...
  const add = async () => {
    ...
    const newMem: Memory = {
      ...
      tags: [], source: "user", pinned: false,
      createdAt: new Date(), updatedAt: new Date(),
    };
    setMemories(m => [newMem, ...m]);
    setNewContent(""); setAdding(false);
    toast.success("Memory saved — Vyom will remember this");
  };
  ...
  const togglePin = async (m: Memory) => {
    if (!user) return;
    haptic(8);
    await updateMemory(user.uid, m.id, m.content);
    // update pin state
    import("firebase/firestore").then(({ doc, updateDoc }) => {
      import("@/integrations/firebase/config").then(({ db }) => {
        updateDoc(doc(db!, "users", user.uid, "memories", m.id), { pinned: !m.pinned });
      });
    });
    setMemories(ms => ms.map(x => x.id === m.id ? { ...x, pinned: !x.pinned } : x));
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-background/95 backdrop-blur-xl animate-fade-in">
      ...
      <div className="flex items-center justify-between px-5 pt-5 pb-3 border-b border-white/8">
        ...
        <button onClick={onClose} className="flex h-9 w-9 items-center justify-center rounded-full hover:bg-white/8">
          <X className="h-4 w-4" />
        </button>
      </div>
      ...
      <div className="px-5 py-3">
        <div className="flex items-center gap-2 glass rounded-2xl px-3 py-2">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)}
            placeholder="Search memories…" className="flex-1 bg-transparent text-sm focus:outline-none" />
        </div>
      </div>
      ...
      {adding ? (
        <div className="px-5 pb-3 space-y-2">
          ...
          <textarea value={newContent} onChange={e => setNewContent(e.target.value)} rows={2}
            placeholder="What should Vyom remember? e.g. I'm preparing for EAMCET 2025"
          ...
          <div className="flex gap-2">
            <button onClick={() => setAdding(false)} className="flex-1 rounded-xl bg-white/5 py-2 text-sm border border-white/10">Cancel</button>
            <button onClick={add} disabled={!newContent.trim()}
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-6 space-y-3">
        {loading ? (
          <div className="text-center text-sm text-muted-foreground pt-8">Loading memories…</div>
        ) : memories.length === 0 ? (
          <div className="flex flex-col items-center justify-center pt-12 text-center gap-3">
            ...
            <p className="text-xs text-muted-foreground/60 max-w-xs">
              Tell Vyom things about yourself — your goals, preferences, and projects — and it will remember them across all conversations.
            </p>
            <div className="flex flex-wrap gap-2 justify-center mt-2">
              {["I'm studying for EAMCET","I prefer concise answers","I'm building a React app","I know Python"].map(ex => (
                <button key={ex} onClick={() => { setNewContent(ex); setAdding(true); }}
                  ...
                  <Sparkles className="h-3 w-3 inline mr-1" />{ex}
                </button>
              ))}
            </div>
          </div>
...
function MemoryCard({ m, onDelete, onPin }: { m: Memory; onDelete: (m: Memory) => void; onPin: (m: Memory) => void }) {
  return (
    <div className={`flex items-start gap-3 rounded-2xl border px-3 py-2.5 ${m.pinned ? "border-white/20 bg-white/8" : "border-white/8 bg-white/4"}`}>
      <span className="text-base mt-0.5 shrink-0">{TYPE_ICONS[m.type]}</span>
      ...
      <div className="flex items-center gap-1 shrink-0">
        <button onClick={() => onPin(m)} className="flex h-7 w-7 items-center justify-center rounded-full hover:bg-white/8 text-muted-foreground hover:text-foreground transition">
          {m.pinned ? <PinOff className="h-3.5 w-3.5" /> : <Pin className="h-3.5 w-3.5" />}
        </button>
        <button onClick={() => onDelete(m)} className="flex h-7 w-7 items-center justify-center rounded-full hover:bg-red-500/10 text-muted-foreground hover:text-red-400 transition">
